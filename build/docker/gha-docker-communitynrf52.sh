...
arduino_compile() {
   ...
   #Compile
   cmdCompile="/usr/local/bin/arduino-cli compile -v --fqbn community_nrf52:nrf52:$fqbn --build-path $buildPath --build-cache-path $buildCachePath $sourcePath/firmware.ino"
    if $verbose; then 
      $cmdCompile
    else
       $cmdCompile > /dev/null
