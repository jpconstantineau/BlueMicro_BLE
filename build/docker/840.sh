...
arduino_compile() {
   ...
   if $failed; then
     ...
     cp -f $buildPath/firmware.ino.elf $outputPath/$keyboard/$keyboard-$keymap-$target.nrf52840.elf
...
printf "\n"
printf "Compiling keyboard:keymap:target  nRF52840\n"
printf -- "-----------------------------------------------------\n"
...
if ((failedBuilds840 != 0 || successfulBuilds840 == 0)); then
   exit 1
fi
